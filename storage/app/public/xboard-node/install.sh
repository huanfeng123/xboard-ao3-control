...
validate_install_request() {
    ...
    if [ -z "$DOWNLOAD_BASE" ]; then
        DOWNLOAD_BASE="${PANEL_URL%/}/storage/xboard-node/releases"
    fi
}
...
resolve_download_url() {
    ...
    if [ "$RELEASE_VERSION" = "latest" ]; then
        DOWNLOAD_URL="${DOWNLOAD_BASE:-$DEFAULT_DOWNLOAD_BASE}/latest/download/${artifact}"
    else
        DOWNLOAD_URL="${DOWNLOAD_BASE:-$DEFAULT_DOWNLOAD_BASE}/download/${RELEASE_VERSION}/${artifact}"
    fi
}
